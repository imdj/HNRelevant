...
class TextAnalyzer {
    ...
    getTermFrequency(text) {
        const tokens = this.cleanText(text).split(' ');
    ...
    extractKeywords(title, comments) {
        const titleTerms = this.getTermFrequency(title);
        const commentTerms = {};

        for (const comment of comments) {
            const termsPerSingleComment = this.getTermFrequency(comment.textContent);
